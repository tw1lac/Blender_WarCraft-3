...
def get_curves(obj, data_path, indices):
    ...
    if obj.animation_data and obj.animation_data.action:
        for index in indices:
            ...
            if curve is not None:
                curves[(data_path.split('.')[-1],
                        index)] = curve  # For now, i'm just interested in the type, not the whole data path. Hence, the split returns the name after the last dot.
    if len(curves):
        return curves
    return None
